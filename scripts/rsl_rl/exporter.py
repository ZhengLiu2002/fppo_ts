...
class _ParkourDeployTorchPolicyExporter(torch.nn.Module):
    def __init__(self, policy, estimator ,normalizer=None):
        ...
        self._num_priv_hurdles = int(getattr(policy, "num_priv_hurdles", 0))
        if self._num_priv_hurdles <= 0 and hasattr(policy, "gating_input_indices"):
            self._num_priv_hurdles = len(getattr(policy, "gating_input_indices"))
        self._start = self._num_prop + self._num_scan + self._num_priv_hurdles
        self._end = self._num_prop + self._num_scan + self._num_priv_explicit
        if normalizer:
    ...
    def forward(self, x, scandots_latent):
        priv_est = self.estimator(x[:, : self._num_prop])
        if priv_est.numel() > 0:
            x[:, self._start : self._end] = priv_est
