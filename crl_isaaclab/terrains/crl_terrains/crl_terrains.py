...
from ..utils import crl_field_to_mesh

if TYPE_CHECKING:
    from . import crl_terrains_cfg
...
def padding_height_field_raw(
    height_field_raw: np.ndarray, cfg: crl_terrains_cfg.ExtremeCRLRoughTerrainCfg
) -> np.ndarray:
...
def random_uniform_terrain(
    difficulty: float,
    cfg: crl_terrains_cfg.ExtremeCRLRoughTerrainCfg,
...
def crl_gap_terrain(
    difficulty: float,
    cfg: crl_terrains_cfg.ExtremeCRLGapTerrainCfg,
    num_goals: int,
    ...
    half_valid_width = round(
        np.random.uniform(cfg.half_valid_width[0], cfg.half_valid_width[1]) / cfg.horizontal_scale
    )
...
@crl_field_to_mesh
def crl_hurdle_terrain(
    difficulty: float,
    cfg: crl_terrains_cfg.ExtremeCRLHurdleTerrainCfg,
    num_goals: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
@crl_field_to_mesh
def crl_step_terrain(
    difficulty: float,
    cfg: crl_terrains_cfg.ExtremeCRLStepTerrainCfg,
    num_goals: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def crl_terrain(
    difficulty: float,
    cfg: crl_terrains_cfg.ExtremeCRLTerrainCfg,
    num_goals: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    width_pixels = int(cfg.size[0] / cfg.horizontal_scale)
    length_pixels = int(cfg.size[1] / cfg.horizontal_scale)
    ...
    last_incline_height = eval(
        cfg.last_incline_height, {"difficulty": difficulty, "incline_height": incline_height}
    )
    ...
    for i in range(num_stones):
        ...
        if i == num_stones - 1:
            ...
            heights = (
                np.tile(np.linspace(-incline_height, incline_height, stone_width), (stone_len, 1))
                * pos_neg
            )
            height_field_raw[
...
@crl_field_to_mesh
def crl_demo_terrain(
    difficulty: float,
    cfg: crl_terrains_cfg.ExtremeCRLDemoTerrainCfg,
    num_goals: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    goals = np.zeros((num_goals, 2))
    width_pixels = int(cfg.size[0] / cfg.horizontal_scale)
