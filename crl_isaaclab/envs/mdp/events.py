...
from crl_isaaclab.actuators import CRLDCMotor
...
def reset_root_state(
    ...
    if min_height is not None:
        min_h = torch.tensor(min_height, device=env.device, dtype=origin.dtype)
        origin[:, 2] = torch.maximum(origin[:, 2], min_h)
...
def randomize_rigid_body_com(
    ...
    # Sample for each environment and each body: (len(env_ids), len(body_ids), 3)
    rand_samples = math_utils.sample_uniform(
        ranges[:, 0], ranges[:, 1], (len(env_ids), len(body_ids), 3), device="cpu"
    )

    # get the current com of the bodies (num_envs, num_bodies, 3)
    coms = asset.root_physx_view.get_coms().clone()
    # Randomize the com in range - only for selected environments and bodies
...
class randomize_rigid_body_material(ManagerTermBase):
    ...
    def __call__(
        ...
        restitution_range: tuple[float, float] | None = None,  # 接受但不使用，仅用于通过参数检查
    ):
        # resolve environment ids
        if env_ids is None:
            env_ids = torch.arange(env.scene.num_envs, device="cpu")
