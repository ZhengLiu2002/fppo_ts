...
class CRLEvent(CRLTerm):
    ...
    def __init__(self, cfg: CRLEventsCfg, env: CRLManagerBasedRLEnv):
        ...
        if self.debug_vis:
            self.total_heights = torch.from_numpy(terrain_generator.goal_heights).to(
                device=self.device
            )
            self.future_goal_idx = torch.ones(
                self.num_goals, device=self.device, dtype=torch.bool
            ).repeat(self.num_envs, 1)
            self.future_goal_idx[:, 0] = False
            self.env_per_heights = self.total_heights[
                self.terrain.terrain_levels, self.terrain.terrain_types
            ]

        self.total_terrain_names = terrain_generator.terrain_names
        numpy_terrain_levels = (
            self.terrain.terrain_levels.detach().cpu().numpy()
        )  ## string type can't convert to torch
        numpy_terrain_types = self.terrain.terrain_types.detach().cpu().numpy()
        self.env_per_terrain_name = self.total_terrain_names[
            numpy_terrain_levels, numpy_terrain_types
        ]
        self._reset_offset = self.env.event_manager.get_term_cfg("reset_root_state").params[
            "offset"
        ]
    ...
    def _gather_cur_goals(self, future=0):
        ...
        return self.env_goals.gather(
            1, idx[:, None, None].expand(-1, -1, self.env_goals.shape[-1])
        ).squeeze(1)
    ...
    def _update_command(self):
        ...
        self.reached_goal_ids = (
            torch.norm(robot_root_pos_w - self.cur_goals[:, :2], dim=1) < self.next_goal_threshold
        )
        ...
        if self.debug_vis and self.reached_goal_ids.any():
            ...
            valid_mask = (
                (self.cur_goal_idx >= 0)
                & (self.cur_goal_idx < self.num_goals)
                & self.reached_goal_ids
            )
            if valid_mask.any():
                # 标记当前目标为已到达（变绿）
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if curriculum_on:
            ...
            recall_mask = (stage_used > 0) & (
                torch.rand_like(stage_used.float()) < self.recall_prob
            )
            ...
            level_values = self.stage_to_level[
                torch.clamp(stage_used, max=self.stage_to_level.shape[0] - 1)
            ]
    ...
    def _offset_goals_from_hurdles(self, goal_tensor: torch.Tensor) -> torch.Tensor:
        ...
        adjusted_x = torch.where(
            goal_x < nearest_hurdle_x, nearest_hurdle_x + margin, nearest_hurdle_x - margin
        )
    ...
    def _apply_hurdle_layout_goals(
        ...
        if layout != "auto" or not curriculum_on:
            visible = torch.full(
                (env_ids.numel(),), hurdle_count, device=self.device, dtype=torch.long
            )
        else:
        ...
        if slot_max > 0:
            hurdle_xs = torch.clamp(
                start + spacing * torch.arange(slot_max, device=self.device, dtype=torch.float),
                min_x,
                max_x,
            )
            mask = torch.arange(slot_max, device=self.device, dtype=torch.long).unsqueeze(
                0
            ) < visible.unsqueeze(1)
    ...
    def _get_last_outcome(self, env_ids: Sequence[int]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if not hasattr(self.env, "reset_time_outs") or self.env.reset_time_outs.numel() == 0:
            return torch.zeros(len(env_ids), device=self.device, dtype=torch.bool), torch.zeros(
                len(env_ids), device=self.device, dtype=torch.bool
            )
        try:
        ...
        reached_goal = (
            self.reached_goal_ids[env_ids]
            if hasattr(self, "reached_goal_ids")
    ...
    def _update_stage(
        self, env_ids: Sequence[int], success_mask: torch.Tensor, valid_mask: torch.Tensor
    ):
        ...
        eval_mask = (self.stage_attempts >= self.stage_eval_window) & (
            self.stage_cooldown >= self.stage_min_stay
        )
        success_rate = torch.where(
            self.stage_attempts > 0,
            self.stage_success.float() / self.stage_attempts.float(),
            torch.zeros_like(self.stage_success, dtype=torch.float),
        )

        promote = eval_mask & (success_rate >= 0.7) & (self.stage < self.max_stage)
        demote = eval_mask & (success_rate <= 0.3) & (self.stage > 0)

        self.stage = torch.clamp(
            self.stage + promote.to(torch.long) - demote.to(torch.long), 0, self.max_stage
        )
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        if debug_vis:
            if not hasattr(self, "current_goal_pose_visualizer"):
                self.current_goal_pose_visualizer = VisualizationMarkers(
                    self.cfg.current_goal_pose_visualizer_cfg
                )
            # set their visibility to true
            self.current_goal_pose_visualizer.set_visibility(True)
            if not hasattr(self, "future_goal_poses_visualizer"):
                self.future_goal_poses_visualizer = VisualizationMarkers(
                    self.cfg.future_goal_poses_visualizer_cfg
                )
            self.future_goal_poses_visualizer.set_visibility(True)

            if not hasattr(self, "current_arrow_visualizer"):
                self.current_arrow_visualizer = VisualizationMarkers(
                    self.cfg.current_arrow_visualizer_cfg
                )
            # set their visibility to true
            self.current_arrow_visualizer.set_visibility(True)
            if not hasattr(self, "future_arrow_visualizer"):
                self.future_arrow_visualizer = VisualizationMarkers(
                    self.cfg.future_arrow_visualizer_cfg
                )
    ...
    def _debug_vis_callback(self, event):
        ...
        # 获取第一个环境的目标点数据
        env_0_goals = self.env_goals[
            env_id : env_id + 1, : self.num_goals
        ]  # shape: (1, num_goals, 3)
        env_0_xy_goals = env_0_goals[:, :, :2] + self.env_origins[
            env_id : env_id + 1, :2
        ].unsqueeze(1)
        env_0_goal_height = (
            self.env_per_heights[env_id : env_id + 1].unsqueeze(-1)
            * self.terrain.cfg.terrain_generator.vertical_scale
        )
        env_0_goal_pos = torch.concat(
            [env_0_xy_goals, env_0_goal_height], dim=-1
        )  # shape: (1, num_goals, 3)

        # 获取第一个环境的目标状态
        env_0_future_goal_idx = self.future_goal_idx[
            env_id : env_id + 1, :
        ]  # shape: (1, num_goals)
        ...
        env_0_next_target_pos_rel = self.next_target_pos_rel[
            env_id : env_id + 1, :
        ]  # shape: (1, 2)
        ...
        for i in range(self.arrow_num):
            ...
            if len(env_0_future_goal_pos) > 0:
                ...
                future_pose_arrow = (
                    env_0_robot_pos_w[:, :2] + 0.2 * (i + 3) * target_vec_norm[:, :2]
                )
