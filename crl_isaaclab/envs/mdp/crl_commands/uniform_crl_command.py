...
class UniformCRLCommand(CommandTerm):
    ...
    def __init__(self, cfg: CRLCommandCfg, env: CRLManagerBasedEnv):
        ...
        # 导入 ConfigSummary 以访问地形特定的指令配置
        try:
            from crl_tasks.crl_task.config.galileo.config_summary import ConfigSummary
            self.terrain_ranges = ConfigSummary.command.ranges
        except ImportError:
            omni.log.warn("Cannot import ConfigSummary, falling back to default ranges")
    ...
    def _get_terrain_name(self, env_ids: Sequence[int]) -> str | None:
        ...
        if hasattr(terrain, "terrain_generator_class"):
            ...
            if hasattr(terrain_gen, "terrain_names") and hasattr(terrain, "terrain_levels") and hasattr(terrain, "terrain_types"):
                try:
                    first_env_id = env_ids[0] if isinstance(env_ids, (list, tuple)) else env_ids[0].item()
    ...
    def _get_terrain_specific_ranges(self, terrain_name: str | None):
        ...
        if terrain_name not in self._terrain_warning_printed:
            omni.log.warn(
                ...
                f"将使用 cfg.ranges 中的默认配置。请确保在 ConfigSummary.command.ranges 中为所有地形添加配置。"
            )
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        if terrain_ranges is not None:
            # 使用地形特定的配置
            lin_vel_x_base = terrain_ranges["lin_vel_x"]
            lin_vel_y_base = terrain_ranges["lin_vel_y"]
            ang_vel_z_base = terrain_ranges["ang_vel_z"]
            heading_base = terrain_ranges.get("heading", self.cfg.ranges.heading)
            heading_command_prob = terrain_ranges.get("heading_command_prob", self.cfg.ranges.heading_command_prob)
            yaw_command_prob = terrain_ranges.get("yaw_command_prob", self.cfg.ranges.yaw_command_prob)
            standing_command_prob = terrain_ranges.get("standing_command_prob", self.cfg.ranges.standing_command_prob)
            start_curriculum_lin_x = terrain_ranges.get("start_curriculum_lin_x", self.cfg.ranges.start_curriculum_lin_x)
            start_curriculum_ang_z = terrain_ranges.get("start_curriculum_ang_z", self.cfg.ranges.start_curriculum_ang_z)
            max_curriculum_lin_x = terrain_ranges.get("max_curriculum_lin_x", self.cfg.ranges.max_curriculum_lin_x)
            max_curriculum_ang_z = terrain_ranges.get("max_curriculum_ang_z", self.cfg.ranges.max_curriculum_ang_z)
        else:
            # 使用默认配置
        ...
        if (
            ...
            start_min, start_max = start_curriculum_lin_x if start_curriculum_lin_x is not None else lin_vel_x_base
            max_min, max_max = max_curriculum_lin_x if max_curriculum_lin_x is not None else lin_vel_x_base
            lin_x_range = (
        ...
        if self.cfg.heading_command and heading_base is not None:
            ...
            heading_prob = heading_command_prob if terrain_ranges is not None else self.cfg.rel_heading_envs
            self.is_heading_env[env_ids] = r.uniform_(0.0, 1.0) <= heading_prob
        # update standing envs
        # 使用地形特定的 standing_command_prob，如果存在；否则使用配置中的值
        standing_prob = standing_command_prob if terrain_ranges is not None else self.cfg.rel_standing_envs
